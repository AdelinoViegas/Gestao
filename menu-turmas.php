...
$sql_turma = "SELECT * FROM sg_turma order by nome_t";
$res_turma = mysqli_query($conexao,$sql_turma);

  if (isset($_POST['btn-pesquisa'])) {
      $pesquisar = $_POST['txtpesquisar'];
      $res_turma = mysqli_query($conexao,"SELECT * FROM sg_turma WHERE nome_t LIKE '$pesquisar%' ");                    
  }
...
<table class="table table-hover table-bordered" id="table">
  ...
  <tbody>
      <?php 
         ...
         if(mysqli_num_rows($res_turma) > 0){

           while($l_turma = mysqli_fetch_assoc($res_turma)) { ?>
     <tr id="tr">
      <td id="editar">
        <form action="turma-editar.php" method="post">
          <input id="editar1" type="hidden" class="btn btn-warning" value="<?php echo $l_turma['id_t']; ?>" name="id_turma">
