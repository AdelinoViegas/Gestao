...
if(isset($_POST['btn-cadastrar'])){
    ...
    $verificar_nome = mysqli_query($conexao,"SELECT * FROM sg_disciplina WHERE nome_d ='$nome'");

if(mysqli_num_rows($verificar_nome) > 0){
     $_SESSION['Disciplina-cadastrado'] = "
   ...
   <div class='alerta-confirmar'>
      <div class='alert alert-danger alert-dimissible'>
       <button style='float:right;' class='btn-close' data-bs-dismiss='alert'></button>
          A disciplina já foi cadastrada!
      </div>
   </div>
   </div>";
    ...
    $sql_disciplina = "INSERT INTO sg_disciplina VALUES (default,'$nome')";

 $r_disciplina = mysqli_query($conexao,$sql_disciplina);

if($r_disciplina == true){
...
<body>
  ...
  <?php
  if(isset($_SESSION['Disciplina-cadastrado'])){
      echo $_SESSION['Disciplina-cadastrado'];
      unset($_SESSION['Disciplina-cadastrado']);
    }  
...
<div class="fontes rounded-3" id="divm">
  ...
  <div class="divsuperior3">
    <h5>Formulário de cadastramento de disciplinas</h5>
      ...
      <form action="disciplina-cadastro.php" method="post">

        <div class="row margB">
         <div class="form-group col-md-4" id="margemB">
            <label for="textnome">Nome</label>
           <input type="text" id="textnome" class="form-control"
           name="txtnome" maxlength="30" placeholder="Nome da disciplina" required>
         </div>
        </div>
        ...
        <div class="row marg">
          ...
          <a href="menu-disciplinas.php" class="btn btn-outline-secondary btn-block col-md-2" 
