...
$sql_disciplina = "SELECT * FROM sg_disciplina order by nome_d";
$res_disciplina = mysqli_query($conexao,$sql_disciplina);

  if (isset($_POST['btn-pesquisa'])) {
      $pesquisar = $_POST['txtpesquisar'];
      $res_disciplina = mysqli_query($conexao,"SELECT * FROM sg_disciplina WHERE nome_d LIKE '$pesquisar%' ");                    
  }
     ...
     <tr id="tr">
      <td id="editar">
        <form action="disciplina-editar.php" method="post">
          <input id="editar1" type="hidden" class="btn btn-warning" value="<?php echo $l_disciplina['id_d']; ?>" name="id_disciplina">
