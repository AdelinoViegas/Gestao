...
$sql_gerenciar = "SELECT * FROM sg_gerenciar AS g JOIN sg_disciplina AS d ON g.idDisciplina = d.id_d JOIN sg_professor AS p ON g.idProfessor = p.id_p JOIN sg_turma AS t ON g.idTurma = t.id_t WHERE view = '1'";
$res_gerenciar = mysqli_query($conexao,$sql_gerenciar);

  if (isset($_POST['btn-pesquisa'])) {
      $pesquisar = $_POST['txtpesquisar'];
      $res_gerenciar = mysqli_query($conexao,"SELECT * FROM sg_gerenciar AS g JOIN sg_disciplina AS d ON g.idDisciplina = d.id_d JOIN sg_professor AS p ON g.idProfessor = p.id_p JOIN sg_turma AS t ON g.idTurma = t.id_t WHERE (nome_p LIKE '$pesquisar%') OR (nome_t LIKE '$pesquisar%') OR (nome_d LIKE '$pesquisar%') AND (view ='1')");                    
  }
...
<table class="table table-hover table-bordered" id="table">
  ...
  <tbody>
      ...
      if(mysqli_num_rows($res_gerenciar) > 0){

          while($l_gerenciar = mysqli_fetch_assoc($res_gerenciar)) { ?>
     <tr id="tr"> 
      <td>
